...
<head>
    ...
    <title>Previous Inquiry - Agency Staff</title>
    <style>
        body {
            height: 100%;
            margin: 0;
        ...
        .no-underline:hover {
            text-decoration: none;
            color: #5a4e7c; /* Slight color change on hover */
        }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-box">
            <!-- ✅ Filter Section -->
            <form method="GET" action="{{ route('InquirySubmission.agencyPreviousInquiry') }}" class="filter-bar">
            ...
            <tr>
                <td>
                    @if(isset($inquiry->submissionID))
                        <a href="{{ route('InquirySubmission.agencyPreviousInquiryDetails', ['id' => $inquiry->submissionID]) }}" class="no-underline">
                            {{ $inquiry->submissionTitle }}
                        </a>
                    @else
                        <p style="color: red;">Error: Inquiry ID missing</p>
                    @endif
                </td>
                <td>{{ $inquiry->verificationStatus }}</td>
                <td>{{ $inquiry->verificationDate }}</td>
                <td>{{ $inquiry->assignmentDate }}</td>
            </tr>
            @endforeach
        </tbody>
        </table>
