...
<head>
    ...
    <title>New Inquiry - MCMC Staff</title>
    <style>
        body {
            height: 100%;
            margin: 0;
...
<body>
    ...
    <div class="main-content">
        <div class="content-header"> New Inquiry</div>
    ...
    @if($inquiries->isNotEmpty())
        ...
        <a href="{{ route('InquirySubmission.mcmcInquiryDetails', ['id' => $inquiry->submissionID]) }}" class="inquiry-link">
            <div class="inquiry-card">
                ...
                <div class="inquiry-actions">
                    <button class="review-btn verify-btn" onclick="markInquiryStatus('{{ $inquiry->submissionID }}', 'verified')">Mark as Verified</button>
                    <button class="review-btn dismiss-btn" onclick="markInquiryStatus('{{ $inquiry->submissionID }}', 'dismissed')">Dismiss Inquiry</button>
                </div>
            </div>
        </a>
        @endforeach
    @else
        <p>No new inquiries to review yet.</p>
    @endif
</div>
</div>
    </div>
    <script>
    function markInquiryStatus(inquiryID, status) {
        fetch("{{ route('InquirySubmission.updateInquiryStatus', ['id' => '__INQUIRY_ID__']) }}".replace('__INQUIRY_ID__', inquiryID), { // Replace placeholder with actual inquiry ID
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                document.querySelector(`.review-btn[onclick='markInquiryStatus("${inquiryID}", "${status}")']`).closest('.inquiry-card').remove();
            }
        })
