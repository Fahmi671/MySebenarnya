...
<head>
    ...
    <title>New Inquiry Details - MCMC Staff</title>
    <style>
        body {
            height: 100%;
            margin: 0;
        ...
        .inquiry-status {
            ...
            min-width: 120px;
            position: relative;
            box-shadow: 0 2px 8px rgba(0,0,0,0.15); /* Adds contrast */
        ...
        .status-pending {
            ...
            color: #222; /* Dark text to contrast yellow */
        }
        ...
        .content-box {
            ...
            transition: all 0.3s ease;
            position: relative;
            z-index: 5; /* Ensure it doesn't override status */
        }
...
<body>
    ...
    <nav class="sidebar" aria-label="Agency sidebar">
        ...
        <ul class="menuitems">
            ...
            <li><a href="{{ route('InquirySubmission.mcmcNewInquiry') }}" class="menuitem active">New Inquiry</a></li>
    ...
    <div class="main-content">
        <div class="content-header">New Inquiry Details</div>
        ...
        <p><strong>Status:</strong>
            <span class="inquiry-status status-{{ strtolower($inquiry->submissionStatus ?? 'pending') }}">
        ...
        <a href="{{ route('InquirySubmission.mcmcNewInquiry') }}" class="btn-back">Back to Inquiry List</a>
    </div>
    </div>
    </div>
    <script>
    function markAsReviewed(inquiryID) {
        fetch(`/mcmc/inquiry/review/${inquiryID}`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                document.querySelector(`.review-btn[onclick='markAsReviewed(${inquiryID})']`).closest('.inquiry-card').remove();
            }
        })
