...
<body>
   <header class="header">
    ...
    <div class="main-content">
        <div class="content-header">Assigned Inquiry</div>
        <div class="content-box">
            <h2 style="margin-top:0;">Assigned Inquiries</h2>
            <table>
                <thead>
                    <tr>
                        <th>Title</th>
                ...
                @forelse($inquiries as $inquiry)
                    @php $assignment = $inquiry->assignments->first(); @endphp
                    <tr>
                        <td>{{ $inquiry->submissionTitle }}</td>
                        <td>{{ $assignment?->assignmentDate ?? '-' }}</td>
                        <td>
                            @if(is_null($assignment?->jurisdictionStatus))
                                <span class="status-pending">Pending</span>
                            @elseif($assignment?->jurisdictionStatus == 'accept')
                                <span class="status-accepted">Accepted</span>
                            @else
                                <span class="status-other">{{ ucfirst($assignment?->jurisdictionStatus) }}</span>
                            @endif
                        </td>
                        <td>
                        <a class="action-link" href="{{ route('InquiryProgress.agencyAssignedInquiryDetails', $assignment?->assignmentID) }}">
                                View
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4">No assigned inquiries found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
